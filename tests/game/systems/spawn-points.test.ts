...
describe('Spawn Points 64x64', () => {
  test('define spawn inicial e checkpoints principais', () => {
    ...
    expect(CHECKPOINTS[4]).toEqual({ id: 4, label: 'Caminho para Residência', posX: 32, posY: 30 });
    expect(CHECKPOINTS[5]).toEqual({ id: 5, label: 'Pátio da Casa de Rafael', posX: 48, posY: 41 });
